...
from .models import Address, UserProfile, ReturnRequest, Wallet, Review
...
class AddressForm(forms.ModelForm):
    ...
    address_type = forms.ChoiceField(
        choices=ADDRESS_TYPES, widget=forms.Select(attrs={"class": "form-control"})
    )
        ...
        if self.user and is_default:
            ...
            if default_exists.exists():
                raise forms.ValidationError(
                    "You already have a default address. Unset it before setting a new one."
                )
...
class EmailChangeForm(forms.Form):
    new_email = forms.EmailField(
        widget=forms.EmailInput(
            attrs={"class": "form-control", "placeholder": "Enter new email address"}
        )
    )
...
class OTPVerificationForm(forms.Form):
    otp_1 = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "otp-input", "maxlength": "1", "pattern": "[0-9]"}
        ),
    )
