...
def toggle_block_user(request, user_id):
    ...
    user.save()
    messages.success(
        request, f"User {'blocked' if user.is_blocked else 'unblocked'} successfully."
    )
...
def edit_product(request, product_id):
    ...
    return render(
        request, "admin_panel/edit_product.html", {"form": form, "product": product}
    )
...
def upload_image(request):
    if request.method == "POST" and request.FILES.get("cropped_image"):
        ...
        final_img = InMemoryUploadedFile(
            buffer,
            None,
            f"cropped_{timezone.now().timestamp()}.jpg",
            "image/jpeg",
            buffer.tell(),
            None,
        )
...
def add_variant(request, product_id):
    ...
    return render(
        request, "admin_panel/add_variant.html", {"form": form, "product": product}
    )
...
def admin_order_list(request):
    ...
    stats_list = [
        ...
        },
        {
            ...
            "count": Order.objects.filter(status="delivered").count(),
            "color": "success",
        },
        {
            "label": "Return Requests",
            "count": Order.objects.filter(returnrequest__isnull=False).count(),
            "color": "danger",
        },
    ]
...
def admin_change_order_status(request, order_id):
    ...
    if request.method == "POST":
        ...
        if new_status in dict(Order.ORDER_STATUS_CHOICES).keys():
            ...
            order.save()
            messages.success(
                request, f"Order {order_id} status updated to {new_status}."
            )
        else:
...
def admin_verify_return_request(request, return_id):
    ...
    if request.method == "POST":
        ...
        if action == "accept":
            ...
            messages.success(
                request,
                f"Return request accepted and ₹{return_request.refund_amount} refunded to wallet.",
            )
        elif action == "reject":
    ...
    return render(
        request, "admin_panel/verify_return.html", {"return_request": return_request}
    )
...
def update_stock(request, variant_id):
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request,
                f'Stock for variant "{variant.variant_name}" updated successfully.',
            )
        except ValueError:
...
def edit_variant(request, variant_id):
    ...
    return render(
        request,
        "admin_panel/edit_variant.html",
        {"form": form, "product": product, "title": "Edit"},
    )
...
def manage_variants(request, product_id):
    ...
    return render(
        request,
        "admin_panel/manage_variants.html",
        {
            "product": product,
            "variants": variants,
        },
...
def approve_return_request(request, request_id):
    ...
    if not return_request.refunded:
        ...
        if order_item:
            ...
            order.items.update(status="Returned")
        ...
        WalletTransaction.objects.create(
            ...
            description=(
                ...
                if not order_item
                else f"Refund for {order_item.product_name} in Order {order.order_id}"
            ),
        )
...
def admin_return_requests(request):
    ...
    return render(
        request,
        "admin_panel/return_request.html",
        {"return_requests": return_requests, "q": query},
    )
...
def create_coupon(request):
    if request.method == "POST":
        ...
        form = CouponForm()
    return render(
        request, "admin_panel/create_coupon.html", {"form": form, "action": "Create"}
    )
...
def edit_coupon(request, coupon_id):
    ...
    return render(
        request, "admin_panel/edit_coupon.html", {"form": form, "coupon": coupon}
    )
...
def sales_report(request):
    ...
    aggregated_data = {
        key: value if value is not None else 0 for key, value in aggregated_data.items()
    }
...
def create_offer(request):
    if request.method == "POST":
        ...
        form = OfferForm()
    return render(
        request, "admin_panel/offer_form.html", {"form": form, "title": "Create Offer"}
    )
...
def edit_offer(request, offer_id):
    ...
    return render(
        request, "admin_panel/offer_form.html", {"form": form, "title": "Edit Offer"}
    )
...
def wallet_transactions_list(request):
    ...
    transactions = WalletTransaction.objects.select_related("wallet__user").order_by(
        "-created_at"
    )

    return render(
        request,
        "admin_panel/wallet_transaction_list.html",
...
def wallet_transaction_detail(request, transaction_id):
    ...
    return render(
        request,
        "admin_panel/wallet_transaction_detail.html",
        {
